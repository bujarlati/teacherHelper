...
- 视频脚本与视频任务：教案中会生成视频脚本和视频提示词；当视频模型 API Key 与模型名已配置时，会自动向硅基流动提交视频生成任务，并记录任务状态和 `requestId`。可在历史记录中刷新任务状态，获取成功视频链接或失败原因。
...
- `npm run test:e2e` 运行 Playwright 本地演示服务器与路径安全检查；互动模板结构由 Vitest 覆盖。
...
文本模型用于教案生成和题目演示分析。视频模型用于根据教案的视频脚本与提示词提交视频生成任务。提交成功后进入“历史记录”，点击视频任务的“刷新状态”即可查询硅基流动状态接口，并把视频链接或失败原因保存到本机。设置会保存在本机，不会提交到仓库。
...
- Playwright 端到端测试检查的是本地演示服务器与路径安全，不会验证真实硅基流动接口。
